...
public class SmoothCameraFollow2D : MonoBehaviour {
	...
	void Update ()
	{
		if (target) {
			...
			Vector3 point = GetComponent<Camera>().WorldToViewportPoint(new Vector3(target.position.x, (target.position.y + 3.6f), target.position.z));
			// la diferencia que existe entre la posición del objeto y la posición que tiene la cámara
			// respecto al objeto
			Vector3 delta = new Vector3(target.position.x, (target.position.y + 3.65f), target.position.z) - GetComponent<Camera>().ViewportToWorldPoint(new Vector3(0.5f, 0.5f, point.z));
